...
Phase 4: TestDino — Test Reporting & Management (Deferred)

Why

Playwright-native reporting + test management platform with Claude Code integration (MCP Server + Playwright Skill). Free tier: 5,000 executions/month, 1 user, 1 project, AI insights included.

Setup (do after all tests are automated)

1.  Reporter already installed: @testdino/playwright (in devDependencies)
2.  Create TestDino account at testdino.com
3.  Generate API token from TestDino settings
4.  Configure reporter in playwright.config.ts — add TestDino alongside HTML reporter
5.  Run tests with: npx tdpw test -t "<API_KEY>" (real-time streaming)
6.  Install Playwright Skill: npx skills add testdino-hq/playwright-skill
7.  Install TestDino MCP Server for querying test results from Claude Code

Additional Integrations (later)

- Connect Jira from TestDino settings (paid plan required for Jira integration)
- TestDino MCP Server for querying test history from Claude Code

---

Phase 5: CI/CD (GitHub Actions)

- ui-tests.yml — push/PR trigger for src/ui/** + tests/ui/**. Installs chromium. Uploads HTML report. Reports to TestDino.
- api-tests.yml — push/PR trigger for src/api/** + tests/api/**. No browser install. Reports to TestDino.
- Both workflows need TESTDINO_API_TOKEN as a GitHub secret.
...
1.  UI specs — bookStore → bookDetails → navigation (one ticket each)
2.  API framework — types → clients → specs (one ticket each)
3.  CI/CD workflows — GitHub Actions for both suites
4.  TestDino setup — account, reporter config, MCP server, Playwright Skill
...
- TestDino dashboard shows test run results synced from CI

Further Enhancements (beyond MVP)

- Add tags to tests based on RPN (e.g., @high, @medium, @low) for selective execution
  eg npx playwright test --grep @high
