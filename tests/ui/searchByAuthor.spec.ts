...
test.describe('COR-4: Search filters books by author name', () => {
  test('should filter books when searching by author name', async ({ bookStorePage }) => {
    ...
    const expectedTitles = BOOKS.filter((b) => b.author === book.author).map((b) => b.title);

    expect(titles.sort(), `Searching "${book.author}" should show their books`).toEqual(
      expectedTitles.sort(),
    );
  });
  ...
  test('should filter books when searching by partial author name', async ({ bookStorePage }) => {
    const book = getRandomBooks(1)[0];
    const partialName = book.author.split(' ')[0];
    await bookStorePage.searchFor(partialName);

    const titles = await bookStorePage.getVisibleBookTitles();
    const expectedTitles = BOOKS.filter((b) =>
      b.author.toLowerCase().includes(partialName.toLowerCase()),
    ).map((b) => b.title);
